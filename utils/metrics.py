            ...
            with torch.no_grad():
                ...
                for i, modality in enumerate(modalities):
                    ...
                    if modality == "text":
                        ...
                        mge_kwargs["txt_patches"] = txt_tokens.float()
                        mge_kwargs["txt_cls"] = txt_cls.float()
                    else:
                        tokens = encoder_method(imgs[i]).float()
                ...
                rgb_query = model.rgb_center.to(device).expand(batch_size, -1)

                fusion_feats = model.mm_fusion(
                    query=rgb_query.float(),
